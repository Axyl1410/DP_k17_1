...
import vn.giadinh.phonghoc.persistence.dao.EditRoomDAO;
...
public class EditRoomUsecase {
    private static final EditRoomGateway GATEWAY;
    public static StatusDTO statusDTO = new StatusDTO();

    static {
        try {
            GATEWAY = new EditRoomDAO();
    ...
    public static StatusDTO execute(RoomDTO roomDTO) {
        try {
            ...
            GATEWAY.updateRoom(roomDTO);
            statusDTO.setStatus(StatusCode.SUCCESS);
            statusDTO.setMessage("Cập nhật phòng học thành công");
