...
public class EditRoomView implements Initializable, Subscriber {
    ...
    // Room ID to edit (passed from ListRoomView)
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        this.model = new EditRoomModel();
        setViewModel(model);
    }

    public void setViewModel(EditRoomModel model) {
        this.model = model;
        model.addSubscriber(this);
    }

    // Method to set room ID to edit (called from ListRoomView)
    public void setRoomIdToEdit(String roomId) {
    ...
    private void setupEventHandlers() {
        ...
        updateButton.setOnAction(event -> handleUpdateRoom());
    ...
    private void loadRoomData(String roomId) {
        try {
            // TODO: Implement loading room data from database
            // For now, we'll just set some default values
            // In a real implementation, you would fetch the room data from the database
            System.out.println("Loading room data for ID: " + roomId);

            // Set default values (these would come from database in real implementation)
            buildingBlockField.setText("Building A");
            areaField.setText("80.0");
            numLightBulbsField.setText("10");
            startDatePicker.setValue(LocalDate.now());
            roomTypeComboBox.setValue(RoomType.LECTURE_HALL.getFullDisplayName());

            // Show appropriate section based on room type
            handleRoomTypeSelection(RoomType.LECTURE_HALL.getFullDisplayName());

        } catch (Exception e) {
            showAlert("Lỗi", "Không thể tải dữ liệu phòng học: " + e.getMessage());
        }
    }
    ...
    private void handleUpdateRoom() {
        try {
            ...
            model.statusDTO = EditRoomController.execute(roomDTO);
            ...
            if (status.equals(StatusCode.SUCCESS)) {
                showAlert("Thành công", "Phòng học đã được cập nhật thành công!");
                handleBack(); // Go back to list view
            } else {
                showAlert("Lỗi", "Không thể cập nhật phòng học: " + message);
            }
    ...
    private void clearForm() {
        // Clear specific fields but keep room ID
        String currentRoomId = roomIdField.getText();
        ...
        hideAllSpecificSections();
        // Restore room ID
        roomIdField.setText(currentRoomId);
    }
